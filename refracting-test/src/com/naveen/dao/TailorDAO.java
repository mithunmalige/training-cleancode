...
import com.naveen.beans.UniformBean;
import com.naveen.connection.GetConnection;
import com.naveen.utilities.Utilities;
...
public class TailorDAO {
	...
	public int prepareTailorReport(int uniformId, ArrayList<String> classes,
			ArrayList<String> sex, ArrayList<String> schoolIds,
		...
		if (createTableAndStoreValues(uniformId, classes, sex, schoolIds)) {
			try {
				...
				if (getNumberOfUniforms(uniformId) <= 3) {
					...
					// Naveen 5th Feb 2015
					String temp = "";
					boolean isColor = checkColorForUniform(uniformId, classes,
							sex, schoolIds);
					if (isColor) {
					...
					String sql = "";
					boolean isDateNotGiven = startDate.equals("")
							|| endDate.equals("");
					if (isDateNotGiven) {
						sql = temp
								...
								+ "in(" + pSchools + ") and class in("
								+ pClasses + ") and sex in(" + pSex
								+ ")  and unid='" + uniformId
								+ "' AND AU.AUID = AUID ))  order by mo.morder";
					} else {
						sql = temp
								+ "from studentmesurementdetails smd, STUDENTMESUREMENT SM, orderdetail od, mesurementorder mo, "
					...
					logger.info("No of recors inserted in tmp_report1 is "
							+ count);
					...
					// there...
					if (count > 0) {
						gc.cs1 = GetConnection.getMySQLConnection()
								.prepareCall(
										"CALL PR_CREATE_INSERT_TAILOR_RPT("
												+ (isColor == true ? "'Y'"
														: "'N'") + ")");
						...
						gc.cs2 = GetConnection.getMySQLConnection()
								.prepareCall(
										"CALL PR_UPDATE_TAILOR_REPORT1_QTY()");
					...
					String sql = temp
							...
							+ "AND  so.oid = od.oid and  so.cancelled='N' and  SO.PAID='Y' AND  od.unid = mo.unid and  smd.mid=mo.mid and"
							+ " smd.smid in(select smid from  studentmesurement where auid in (select auid from assignuniform  where schid "
							+ "in(" + pSchools + ") and class in(" + pClasses
							+ ") and sex in(" + pSex + ")  and unid='"
							+ uniformId + "' AND AU.AUID = AUID )) "
							+ "order by mo.morder";
					...
					if (count > 0) {
						gc.cs1 = GetConnection.getMySQLConnection()
								.prepareCall(
										"CALL PR_CREATE_INSERT_TAILOR_RPT_ORDERID("
												+ (isColor == true ? "'Y'"
														: "'N'") + ")");
						gc.cs1.executeUpdate();
						// added newly
						gc.cs2 = GetConnection
								.getMySQLConnection()
								.prepareCall(
										"CALL PR_UPDATE_TAILOR_REPORT1_QTY_ORD()");
						gc.cs2.executeUpdate();
	...
	// by naveen
	// the user can send multiple schools or single schools
	public int prepareTailorReportOrderNumber(int uniformId, ArrayList<String> classes,
			ArrayList<String> sex, ArrayList<String> schoolIds,
			String startDate, String endDate, int startOrderNumber, int endOrderNumber) {
				...
				if (getNumberOfUniforms(uniformId) <= 3) {
					...
					if (isDateNotGiven) {
						sql = temp
								+ "from studentmesurementdetails smd, STUDENTMESUREMENT SM, orderdetail od, mesurementorder mo, "
								+ "studentorder so, ASSIGNUNIFORM AU  where OD.QTY > 0 and  SM.SMID = SMD.SMID AND  SM.AUID = AU.AUID AND  OD.SMID = SM.SMID "
								+ "AND  so.oid = od.oid and  so.cancelled='N' and  SO.PAID='Y' AND  od.unid = mo.unid and so.oidn between "+startOrderNumber+" and "+endOrderNumber+"   and  smd.mid=mo.mid and"
								+ " smd.smid in(select smid from  studentmesurement where auid in (select auid from assignuniform  where schid "
								+ "in(" + pSchools + ") and class in("
								+ pClasses + ") and sex in(" + pSex
								+ ")  and unid='" + uniformId
	...
	public boolean createTableAndStoreValues(int uniformId,
			ArrayList<String> classes, ArrayList<String> sex,
	 ...
	 * Naveen - 5th Feb 2015
	 */
	public ArrayList<ArrayList<String>> generateTailorReport(int uniformId,
			ArrayList<String> classes, ArrayList<String> sex,
		...
		if (prepareTailorReport(uniformId, classes, sex, schoolIds, startDate,
				endDate) > 0) {
			...
			if (checkColorForUniform(uniformId, classes, sex, schoolIds)) {
				...
				try {
					if (noOfUniforms <= 3) {
						...
						// commented by naveen
						/*
						 * while(gc.rs1.next()){ ArrayList<String> tempList =
						 * new ArrayList<String>(); for (int i = 1; i <
						 * columnCount; i++) {
						 * 
	...
	/**
	 * Naveen - 5th Feb 2015, edited with startOrderNumber and endOrderNumber on 04 Oct 2017 
	 */
	public ArrayList<ArrayList<String>> generateTailorReportOrderNumber(int uniformId,
			ArrayList<String> classes, ArrayList<String> sex,
		...
		if (prepareTailorReportOrderNumber(uniformId, classes, sex, schoolIds, startDate,
				endDate, startOrderNumber, endOrderNumber) > 0) {
	...
	public boolean checkColorForUniform(int uniformId,
			ArrayList<String> classes, ArrayList<String> sex,
			ArrayList<String> schoolIds) {
		...
		try {
			...
			String sql = "select count(distinct color) from studentmesurement where smid in("
					+ "select smid from orderdetail where auid in (select auid from assignuniform"
					+ " where schid in("
					+ pSchools
					+ ") and class in("
					+ pClasses
	...
	public int getNumberOfUniforms(int uniformId) {
		...
		/*
		 * String pSchools = ""; for (String temp : schoolIds) { pSchools =
		 * pSchools + "'" + temp + "',"; }
		 * 
		 * pSchools = pSchools.substring(0, pSchools.length() - 1);
		 * 
		 * String pClasses = ""; for (String temp : classes) { pClasses =
		 * pClasses + "'" + temp + "',";
		 * 
		 * } pClasses = pClasses.substring(0, pClasses.length() - 1);
		 * 
		 * String pSex = ""; // assuming that boy and girl is given if
		 * (sex.size() > 1) { pSex = "'B', 'G'"; } else { pSex = "'" +
		 * sex.get(0) + "'"; }
		 * 
		 ...
		 * + "where schid in(" + pSchools + ") and class in(" + pClasses +
		 * ") and sex in(" + pSex + ")  and unid='" + uniformId +
		 * "' AND AU.AUID = AUID ))";
		 */
	 ...
	 * 
	 * @param args
	 * @see This method does not return any thing, but it will insert max number
	 *      of counts of the uniform for ex: if skirt generally length & waist
	 *      is taken and hip is left as it is, for few Children's we will take
	 *      hip, if you don't take hip, in the tmp_report1 table only length &
	 *      waist this method will insert 0.1 for those who has not got that
	 *      values
	 */
...
//		
	...
	public ArrayList<ArrayList<Object>> getTailorReportBeforeOrder(int schoolId,
			String cls, int uniformId, String sex) {
		logger.info("Schoold id got is " + schoolId +", Class " + cls +", Uniform Id " + uniformId +", for getTrailorReportBeforeOrder");
		...
		// this query will help to generate group_concat query to know how many measurements are there 
		// for the uniform 
...
//		    FROM deletetable group by stid;
			...
			String sql5 ="select count(*)," + attributes + " from GENERATE_REPORT_BEFORE_ORDER " + 
			" group by "+ attributes +" order by count(*)"; 

			
//			String sql5 ="select count(*)," + attributes + " from GENERATE_REPORT_BEFORE_ORDER where " + 
//			measurementNames.get(0)  +" > 0.5 group by "+ attributes +" order by count(*)"; 
	...
	public void deleteExtraMeasurement(int schoolId, String cls, String sex){
		...
		try {
			...
			for(UniformBean uniform: uniformList){
				// this query will run  n number of times for each uniform fetched for schoo, class, sex 
