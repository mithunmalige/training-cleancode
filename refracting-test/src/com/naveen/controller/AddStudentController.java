...
public class AddStudentController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(new StudentDAO().insertStudent(studb,((LoginBean)request.getSession().getAttribute("LOGIN")).getUserId())){
			if(request.getParameter("formname").equalsIgnoreCase("order")){
			getServletContext().getRequestDispatcher("/MakeOrder2.jsp?msg=Student Added Successfully&schid="+schid+"&class="+cls).forward(request, response);
			}
			else if(request.getParameter("formname").equalsIgnoreCase("student")){
				getServletContext().getRequestDispatcher("/student.jsp?msg=Student Added Successfully&schid="+schid+"&class1="+cls+"&schname="+schname).forward(request, response);
				}
			if(request.getParameter("formname").equalsIgnoreCase("mesurement")){
				getServletContext().getRequestDispatcher("/TakeMesurementForm2.jsp?msg=Student Added Successfully&schoolid="+schid+"&class="+cls).forward(request, response);
				}
			}
		
		else{
			getServletContext().getRequestDispatcher("/student.jsp?msg=Sorry Student Not Added.. Try Again").include(request, response);
