...
public class StudentDAO {
	 ...
	 * @return
	 * 
	 * 		THIS QUERY will give the uniform(s) for the student selected
	 *         mapping with assignuniform table
	 */
	 ...
	 * 
	 * 		THIS QUERY will give the uniform(s) for the student selected
	 *         mapping with assignuniform table used to show student
	 *         measurements while taking order
	 */
	 ...
	 *         MEASUREMENTDATE is null in the DB, if not null then the
	 *         measurement is taken if not he/she was absent on particular day
	 */
	 ...
	 * @param schid
	 * @return
	 * @see : This method is used in  ShowPackingOrder.jsp, for showing student details while packing, this was part of new 
	 * requirement on 22 feb 2018  
	 */
	...
	/*
	 * varish to list order placed students used in packing report, 
	 * added by naveen as part of the new requirement, this method shall take the sex as the parameter 
	 * 21 Dec 2017 
	 * 
	 */
	public List<StudentBean> getOrderPlacedStudentsOfSchool(int schId, String cls, String sdate, String edate, String sex) {
...
String sql1 = "select st.stid,st.usn,st.name,st.sex from student st,studentorder so where st.stid=so.stid and "
	...
	public List<StudentBean> getOrderPlacedStudentsOfSchoolUnid(int schId, int unid, String sdate, String edate,
			String cls) {
		/*String sql1 = "select st.stid,st.usn,st.name,st.sex from student st,studentorder so, orderdetail od where st.stid=so.stid "
	 ...
	 *         details when order id between range is passed, 
	 *         getting the list of all students who placed an order from method 
	...
	public List<StudentBean> getOrderPlacedStudentsOfSchoolUnidWithOrderNo(int schId, int unid, String sdate,
		...
		for (StudentBean temp : list) {
			// get only number part of the oid such that filters between minOrderNo and maxOrderNo can be applied
			String orderId  = temp.getOrderId().substring(temp.getOrderId().lastIndexOf("/")+1, temp.getOrderId().length());
			// since the data got in String format converting to int, and comparing between min and max values 
