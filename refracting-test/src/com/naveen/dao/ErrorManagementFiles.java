...
public class ErrorManagementFiles {
	...
	// this method was create on 02-02-2018 since the amount calculated was amount+tax 
	// and gross amount was supposed to be amount + tax and it happened amount + tax + tax 
		...
		try {
			...
			for(String tempOrderId : orderNumbers){
				...
				while(gc.rs2.next()){
					...
					System.out.println("odid " + tempOrderDetailBean.getOdid() +  ", " +" cost " + amount + "CGST Amount " + cgsTaxAmount +", sgst amount " + sgsTaxAmount +", igst amount " + igsTaxAmount);
